...
def train_all(input_data, output_data, voc_len, chunk_len,
              n_epochs=300,
              hidden_size=100,
              n_layers=1,
              lr=0.015):
...
# build entire model
def build_experience_model(data_path):
    ...
    trained_model = train_all(inp, tar, voc_len, chunk_len)
...
def test_model():
    ...
    the_model.load_state_dict(torch.load(".\\pytorch_models\experience_model.pth"))
...
if __name__ == '__main__':
    #build_experience_model(".\\pytorch_models\experience_model.pth")
    test_model()
